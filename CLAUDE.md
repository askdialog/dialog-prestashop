...
- **HTTP calls must go through `Service\Http\HttpTransport`, never Symfony directly.** `Symfony\Component\HttpClient` and `Symfony\Component\Mime` are Symfony 4.3+ components: PrestaShop only ships them from 8.0, and 1.7.6–1.7.8 (Symfony 3.4) do not have them. Referencing them there raises `Class not found` — a fatal, not an exception, which escapes `catch (Exception)` and takes the front-office page down (it did, on an add-to-cart hook). HttpTransport uses Symfony when available and falls back to cURL otherwise, so the module works on every supported version. Front-office hooks additionally catch `\Throwable`, not `\Exception`, so analytics can never break a page.
